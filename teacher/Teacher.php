...
class Teacher {
	...
	public function register($firstName, $lastName, $password, $email, $gender, $dob, $nationality, $jlpt, $experience, $phone, $api_id, $description) {
		...
		if ($count_row == 0) {
			$insert = "INSERT INTO teachers SET first_name='$firstName',last_name='$lastName', password='$password', email='$email', gender='$gender', dob='$dob', nationality='$nationality', jlpt = '$jlpt', experience='$experience', phone='$phone', api_id = '$api_id', description ='$description'";
	...
	public function update_details($id, $first_name, $last_name, $jlpt, $experience, $phone, $api_id, $description) {

		$update = "UPDATE `teachers` SET `first_name`='$first_name',`last_name`='$last_name',`jlpt`='$jlpt',`experience`='$experience',`description`='$description', api_id= '$api_id' , `phone`='$phone'  WHERE `id` = '$id'";
 ...
 ";
	...
	public function cancel_event($event_id) {
		...
		$result = "DELETE FROM `events` WHERE id='$event_id'";
	...
	public function change_password($current, $new, $confirm) {
		...
		$id = $this->id;
		$result = "SELECT password from teachers WHERE id='$id'";
		$new = md5($new);
		$confirm = md5($confirm);
		$current = md5($current);
		//checking if the username is available in the table
